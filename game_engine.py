# This file is part of TextQuest activity for SugarLabs,
...
class Game:
    def __init__(self, map_file, game_window):
        ...
        start_room = (self.data["currentRoom"])
        self.print(self.data['rooms'][start_room].get('description'))
    ...
    def load_map(self, filename):
        self.copy_filename = 'tmp_' + filename
        if os.path.exists(self.copy_filename):
            try:
                ...
                raise ValueError(f'Issue with copy_{filename}')
        else:
            try:
                shutil.copyfile(filename, self.copy_filename)
                with open('fasdfa', 'r') as f:
                    game_data = json.load(f)
                    return game_data
            except FileNotFoundError:
                raise FileNotFoundError('Map file not found.')
            except json.JSONDecodeError:
                raise ValueError('Error decoding the map file.')

    """Write changes in game state back to the JSON file"""
    ...
    def move_player(self, direction):
        new_room_name = self.current_room.check_deadend(direction)
        if new_room_name:
            ...
            if required_item:
                if self.data.get("inventory"):
                    if required_item not in self.data.get("inventory"):
                        ...
                        return
                    self.print(self.data['rooms'][new_room_name].get('entryMsg'))
    ...
    """Define interactions with the enemies-npcs
        ...
        if npc_type:
            if self.data['npcs'][npc_type].get('toDefeat') in self.data["inventory"]:
                ...
                if npc_type == "enemies":
                    ...
                    room.pop('npcType', None)
                    room['description'] = self.data['rooms'][room_name].get(
                        'emptyDescription')
                    room.pop('warning', None)
                    self.modify_map(self.data)
            else:
                if npc_type == "enemies":
    ...
    """Allow player to examine their surroundings,
    ...
    """Print to gtk.textview instead of console"""
    def print(self, text):
    ...
    def restart_game(self):
        ...
        self.current_room = Room(
            self.data["rooms"].get(start_room_name), self.data, self)
        self.game_window.clear_textview()
        self.print("Game has successfully been restarted")
        self.current_room.describe_room()

    """
    Go to menu without deleting self.copy_filename
...
class Room:
    ...
    """Describe room and items in it"""
    def describe_room(self):
        if self.room_data.get('npcType'):
            npc_description = (
                self.data['npcs'][self.room_data['npcType']].get('description')
            )
            if npc_description:
    ...
    def check_deadend(self, direction):
...
class Player:
    ...
    """Allow player to use an item from their inventory.
        ...
        if item in self.data['inventory']:
            ...
            if npc_type == 'traders':
                if item == self.data['npcs'][npc_type]['desires']:
                    ...
                    self.game.modify_map(self.data)
                    self.game.print(
                        f"You successfully traded your {item} with the {sells_item}!")
            else:
                self.game.print(
                    f"You can't use your {item} here, try in a different room!")
        else:
